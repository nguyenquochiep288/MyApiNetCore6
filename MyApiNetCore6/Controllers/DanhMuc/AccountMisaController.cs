...
namespace MyApiNetCore6.Controllers
{
	...
	public class AccountMisaController : ControllerBase
	{
		...
		public async Task<IActionResult> GetCompany()
		{
			try
			{
				List<dm_TaiKhoan_Misa> lstValue = await _context.dm_TaiKhoan_Misa.ToListAsync();
		...
		public async Task<IActionResult> GetCompany(int Type, string KeyWhere = "", string ValuesSearch = "")
		{
			try
			{
				...
				List<dm_TaiKhoan_Misa> lstValue = await _context.dm_TaiKhoan_Misa.Where(KeyWhere, ValuesSearch).ToListAsync();
				return Ok(new ApiResponse
				{
					Success = true,
					Message = "Success",
		...
		public async Task<IActionResult> GetCompany(string ID)
		{
			try
			{
				dm_TaiKhoan_Misa Company = await _context.dm_TaiKhoan_Misa.FirstOrDefaultAsync((dm_TaiKhoan_Misa e) => e.ID == ID);
				if (Company == null)
				{
					return Ok(new ApiResponse
					{
		...
		public async Task<IActionResult> PutCompany(string MA, dm_TaiKhoan_Misa Company)
		{
			try
			{
				...
				dm_TaiKhoan_Misa TaiKhoan_Misa = await _context.dm_TaiKhoan_Misa.FirstOrDefaultAsync((dm_TaiKhoan_Misa e) => e.ID == Company.ID);
				...
				dm_TaiKhoan_Misa OKCompany = await _context.dm_TaiKhoan_Misa.FirstOrDefaultAsync((dm_TaiKhoan_Misa e) => e.ID == Company.ID);
				return Ok(new ApiResponse
				{
					Success = true,
					Message = "Success",
		...
		public async Task<ActionResult<dm_TaiKhoan_Misa>> PostCompany(dm_TaiKhoan_Misa Company)
		{
			try
			{
				...
				_context.dm_TaiKhoan_Misa.Add(Company);
				AuditLogController auditLog = new AuditLogController(_context, _configuration);
				auditLog.InserAuditLog();
				await _context.SaveChangesAsync();
				dm_TaiKhoan_Misa OKCompany = await _context.dm_TaiKhoan_Misa.FirstOrDefaultAsync((dm_TaiKhoan_Misa e) => e.ID == Company.ID);
		...
		public async Task<IActionResult> DeleteCompany(string ID)
		{
			try
			{
				...
				_context.dm_TaiKhoan_Misa.Remove(Company);
				AuditLogController auditLog = new AuditLogController(_context, _configuration);
				auditLog.InserAuditLog();
				await _context.SaveChangesAsync();
				return Ok(new ApiResponse
		...
		{
			return _context.dm_TaiKhoan_Misa.Any((dm_TaiKhoan_Misa e) => e.ID == ID);
		}

		private bool CompanyExistsMA(string MA)
		{
			return _context.dm_TaiKhoan_Misa.Any((dm_TaiKhoan_Misa e) => e.MASOTHUE == MA);
		}

		private bool CompanyExists(dm_TaiKhoan_Misa Company)
		{
			return _context.dm_TaiKhoan_Misa.Any((dm_TaiKhoan_Misa e) => e.MASOTHUE == Company.MASOTHUE && e.ID != Company.ID);
		}
	}
}
