...
namespace MyApiNetCore6.Controllers
{
	...
	public class TypeInvoicedController : ControllerBase
	{
		...
		public async Task<IActionResult> GetTypeInput(string LOC_ID)
		{
			try
			{
				List<dm_LoaiHoaDon> lstValue = await (from e in _context.dm_LoaiHoaDon
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
		{
			try
			{
				...
				List<dm_LoaiHoaDon> lstValue = await (from e in _context.dm_LoaiHoaDon.Where((dm_LoaiHoaDon e) => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch)
													  orderby e.MA
													  select e).ToListAsync();
				return Ok(new ApiResponse
				{
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID, string ID)
		{
			try
			{
				dm_LoaiHoaDon TypePayment = await _context.dm_LoaiHoaDon.FirstOrDefaultAsync((dm_LoaiHoaDon e) => e.LOC_ID == LOC_ID && e.ID == ID);
				if (TypePayment == null)
				{
					return Ok(new ApiResponse
					{
		...
		[HttpPut("{LOC_ID}/{MA}")]
		[Authorize(Roles = "User")]
		public async Task<IActionResult> PutTypePayment(string LOC_ID, string MA, dm_LoaiHoaDon TypePayment)
		{
			try
			{
		...
		public async Task<ActionResult<dm_LoaiHoaDon>> PostTypePayment(dm_LoaiHoaDon TypePayment)
		{
			try
			{
				...
				_context.dm_LoaiHoaDon.Add(TypePayment);
				AuditLogController auditLog = new AuditLogController(_context, _configuration);
				auditLog.InserAuditLog();
				await _context.SaveChangesAsync();
				return Ok(new ApiResponse
		...
		public async Task<IActionResult> DeleteTypePayment(string LOC_ID, string ID)
		{
			try
			{
				...
				_context.dm_LoaiHoaDon.Remove(TypePayment);
				AuditLogController auditLog = new AuditLogController(_context, _configuration);
				auditLog.InserAuditLog();
				await _context.SaveChangesAsync();
				return Ok(new ApiResponse
		...
		{
			return _context.dm_LoaiHoaDon.Any((dm_LoaiHoaDon e) => e.LOC_ID == LOC_ID && e.MA == MA);
		}

		private bool TypePaymentExistsID(string LOC_ID, string ID)
		{
			return _context.dm_LoaiHoaDon.Any((dm_LoaiHoaDon e) => e.LOC_ID == LOC_ID && e.ID == ID);
		}

		private bool TypePaymentExists(dm_LoaiHoaDon Position)
		{
			return _context.dm_LoaiHoaDon.Any((dm_LoaiHoaDon e) => e.LOC_ID == Position.LOC_ID && e.MA == Position.MA && e.ID != Position.ID);
		}
	}
}
