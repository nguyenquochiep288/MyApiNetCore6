...
namespace MyApiNetCore6.Controllers
{
	...
	public class TypeOutputController : ControllerBase
	{
		...
		public async Task<IActionResult> GetTypeOutput(string LOC_ID)
		{
			try
			{
				List<dm_LoaiPhieuXuat> lstValue = await (from e in _context.dm_LoaiPhieuXuat
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
		{
			try
			{
				...
				List<dm_LoaiPhieuXuat> lstValue = await (from e in _context.dm_LoaiPhieuXuat.Where((dm_LoaiPhieuXuat e) => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch)
														 orderby e.MA
														 select e).ToListAsync();
				return Ok(new ApiResponse
				{
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID, string ID)
		{
			try
			{
				dm_LoaiPhieuXuat TypePayment = await _context.dm_LoaiPhieuXuat.FirstOrDefaultAsync((dm_LoaiPhieuXuat e) => e.LOC_ID == LOC_ID && e.ID == ID);
				if (TypePayment == null)
				{
					return Ok(new ApiResponse
					{
		...
		{
			return _context.dm_LoaiPhieuXuat.Any((dm_LoaiPhieuXuat e) => e.LOC_ID == LOC_ID && e.MA == MA);
		}

		private bool TypePaymentExistsID(string LOC_ID, string ID)
		{
			return _context.dm_LoaiPhieuXuat.Any((dm_LoaiPhieuXuat e) => e.LOC_ID == LOC_ID && e.ID == ID);
		}

		private bool TypePaymentExists(dm_LoaiPhieuXuat Position)
		{
			return _context.dm_LoaiPhieuXuat.Any((dm_LoaiPhieuXuat e) => e.LOC_ID == Position.LOC_ID && e.MA == Position.MA && e.ID != Position.ID);
		}
	}
}
