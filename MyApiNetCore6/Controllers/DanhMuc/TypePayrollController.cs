    ...
    public class TypePayrollController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetTypeInput(string LOC_ID)
        {
            try
            {
                ...
                var lstValue = await _context.dm_LoaiLuong!.Where(e => e.LOC_ID == LOC_ID).OrderBy(e => e.MA).ToListAsync();
        ...
        public async Task<IActionResult> GetTypePayment(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
        {
            try
            {
                ...
                var lstValue = await _context.dm_LoaiLuong!.Where(e => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch).OrderBy(e => e.MA).ToListAsync();
                return Ok(new ApiResponse
                {
                    Success = true,
                    Message = "Success",
        ...
        public async Task<IActionResult> GetTypePayment(string LOC_ID, string ID)
        {
            try
            {
                var TypePayment = await _context.dm_LoaiLuong!.FirstOrDefaultAsync(e => e.LOC_ID == LOC_ID && e.ID == ID);
        ...
        [HttpPut("{LOC_ID}/{MA}")]
        [Authorize(Roles = UserRoles.User)]
        public async Task<IActionResult> PutTypePayment(string LOC_ID, string MA, dm_LoaiLuong TypePayment)
        {
            try
            {
        ...
        public async Task<ActionResult<dm_LoaiLuong>> PostTypePayment(dm_LoaiLuong TypePayment)
        {
            try
            {
                ...
                _context.dm_LoaiLuong!.Add(TypePayment);
                AuditLogController auditLog = new AuditLogController(_context, _configuration);auditLog.InserAuditLog();await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> DeleteTypePayment(string LOC_ID, string ID)
        {
            try
            {
                ...
                _context.dm_LoaiLuong!.Remove(TypePayment);
                AuditLogController auditLog = new AuditLogController(_context, _configuration);auditLog.InserAuditLog();await _context.SaveChangesAsync();
                return Ok(new ApiResponse
                {
                    Success = true,
        ...
        {
            return _context.dm_LoaiLuong!.Any(e => e.LOC_ID == LOC_ID && e.MA == MA);
        }

        private bool TypePaymentExistsID(string LOC_ID, string ID)
        {
            return _context.dm_LoaiLuong!.Any(e => e.LOC_ID == LOC_ID && e.ID == ID);
        }
        private bool TypePaymentExists(dm_LoaiLuong Position)
        {
            return _context.dm_LoaiLuong!.Any(e => e.LOC_ID == Position.LOC_ID && e.MA == Position.MA && e.ID != Position.ID);
        }
    }
}