...
namespace MyApiNetCore6.Controllers
{
	...
	public class TypePayrollController : ControllerBase
	{
		...
		public async Task<IActionResult> GetTypeInput(string LOC_ID)
		{
			try
			{
				List<dm_LoaiLuong> lstValue = await (from e in _context.dm_LoaiLuong
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
		{
			try
			{
				...
				List<dm_LoaiLuong> lstValue = await (from e in _context.dm_LoaiLuong.Where((dm_LoaiLuong e) => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch)
													 orderby e.MA
													 select e).ToListAsync();
				return Ok(new ApiResponse
				{
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID, string ID)
		{
			try
			{
				dm_LoaiLuong TypePayment = await _context.dm_LoaiLuong.FirstOrDefaultAsync((dm_LoaiLuong e) => e.LOC_ID == LOC_ID && e.ID == ID);
				if (TypePayment == null)
				{
					return Ok(new ApiResponse
					{
		...
		{
			return _context.dm_LoaiLuong.Any((dm_LoaiLuong e) => e.LOC_ID == LOC_ID && e.MA == MA);
		}

		private bool TypePaymentExistsID(string LOC_ID, string ID)
		{
			return _context.dm_LoaiLuong.Any((dm_LoaiLuong e) => e.LOC_ID == LOC_ID && e.ID == ID);
		}

		private bool TypePaymentExists(dm_LoaiLuong Position)
		{
			return _context.dm_LoaiLuong.Any((dm_LoaiLuong e) => e.LOC_ID == Position.LOC_ID && e.MA == Position.MA && e.ID != Position.ID);
		}
	}
}
