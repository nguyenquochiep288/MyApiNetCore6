...
namespace MyApiNetCore6.Controllers
{
	...
	public class TypePaymentController : ControllerBase
	{
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID)
		{
			try
			{
				List<dm_LoaiPhieuChi> lstValue = await (from e in _context.dm_LoaiPhieuChi
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
		{
			try
			{
				...
				List<dm_LoaiPhieuChi> lstValue = await (from e in _context.dm_LoaiPhieuChi.Where((dm_LoaiPhieuChi e) => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch)
														orderby e.MA
														select e).ToListAsync();
				return Ok(new ApiResponse
				{
		...
		public async Task<IActionResult> GetTypePayment(string LOC_ID, string ID)
		{
			try
			{
				dm_LoaiPhieuChi TypePayment = await _context.dm_LoaiPhieuChi.FirstOrDefaultAsync((dm_LoaiPhieuChi e) => e.LOC_ID == LOC_ID && e.ID == ID);
				if (TypePayment == null)
				{
					return Ok(new ApiResponse
					{
		...
		{
			return _context.dm_LoaiPhieuChi.Any((dm_LoaiPhieuChi e) => e.LOC_ID == LOC_ID && e.MA == MA);
		}

		private bool TypePaymentExistsID(string LOC_ID, string ID)
		{
			return _context.dm_LoaiPhieuChi.Any((dm_LoaiPhieuChi e) => e.LOC_ID == LOC_ID && e.ID == ID);
		}

		private bool TypePaymentExists(dm_LoaiPhieuChi Position)
		{
			return _context.dm_LoaiPhieuChi.Any((dm_LoaiPhieuChi e) => e.LOC_ID == Position.LOC_ID && e.MA == Position.MA && e.ID != Position.ID);
		}
	}
}
