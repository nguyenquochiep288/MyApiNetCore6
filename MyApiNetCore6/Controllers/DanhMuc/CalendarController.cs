...
namespace MyApiNetCore6.Controllers
{
	...
	public class CalendarController : ControllerBase
	{
		...
		public async Task<IActionResult> GetArea(string LOC_ID)
		{
			try
			{
				List<dm_LichLamViec> lstValue = await (from e in _context.dm_LichLamViec
		...
		public async Task<IActionResult> GetArea(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
		{
			try
			{
				...
				List<dm_LichLamViec> lstValue = await (from e in _context.dm_LichLamViec.Where((dm_LichLamViec e) => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch)
													   orderby e.MA
													   select e).ToListAsync();
				return Ok(new ApiResponse
				{
		...
		public async Task<IActionResult> GetArea(string LOC_ID, string ID)
		{
			try
			{
				dm_LichLamViec Area = await _context.dm_LichLamViec.FirstOrDefaultAsync((dm_LichLamViec e) => e.LOC_ID == LOC_ID && e.ID == ID);
				if (Area == null)
				{
					return Ok(new ApiResponse
					{
		...
		public async Task<IActionResult> PutArea(string LOC_ID, string MA, dm_LichLamViec Area)
		{
			try
			{
				...
				dm_LichLamViec OKArea = await _context.dm_LichLamViec.FirstOrDefaultAsync((dm_LichLamViec e) => e.LOC_ID == Area.LOC_ID && e.ID == Area.ID);
				return Ok(new ApiResponse
				{
					Success = true,
					Message = "Success",
		...
		public async Task<IActionResult> DeleteArea(string LOC_ID, string ID)
		{
			try
			{
				...
				ApiResponse apiResponse = await ExecuteStoredProc.CheckDelete(Area, Area.ID, Area.MA);
				...
				List<web_PhanQuyenKhuVuc> lstweb_PhanQuyenSanPham = await _context.web_PhanQuyenKhuVuc.Where((web_PhanQuyenKhuVuc e) => e.LOC_ID == LOC_ID && e.ID_LICHLAMVIEC == ID).ToListAsync();
		...
		{
			return _context.dm_LichLamViec.Any((dm_LichLamViec e) => e.LOC_ID == LOC_ID && e.MA == MA);
		}

		private bool AreaExistsID(string LOC_ID, string ID)
		{
			return _context.dm_LichLamViec.Any((dm_LichLamViec e) => e.LOC_ID == LOC_ID && e.ID == ID);
		}

		private bool AreaExists(dm_LichLamViec Area)
		{
			return _context.dm_LichLamViec.Any((dm_LichLamViec e) => e.LOC_ID == Area.LOC_ID && e.MA == Area.MA && e.ID != Area.ID);
		}
	}
}
