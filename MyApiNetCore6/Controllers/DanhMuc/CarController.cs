    ...
    public class CarController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetCar(string LOC_ID)
        {
            try
            {
                ...
                var lstValue = await _context.dm_Xe!.Where(e => e.LOC_ID == LOC_ID).OrderBy(e => e.MA).ToListAsync();
        ...
        public async Task<IActionResult> GetCar(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
        {
            try
            {
                ...
                var lstValue = await _context.dm_Xe!.Where(e => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch).OrderBy(e => e.MA).ToListAsync();
                return Ok(new ApiResponse
                {
                    Success = true,
                    Message = "Success",
        ...
        public async Task<IActionResult> GetCar(string LOC_ID, string ID)
        {
            try
            {
                var Car = await _context.dm_Xe!.FirstOrDefaultAsync(e => e.LOC_ID == LOC_ID && e.ID == ID);
        ...
        public async Task<IActionResult> PutCar(string LOC_ID, string MA, dm_Xe Car)
        {
            try
            {
                ...
                var OKCar = await _context.dm_Xe!.FirstOrDefaultAsync(e => e.LOC_ID == Car.LOC_ID && e.ID == Car.ID);
                return Ok(new ApiResponse
                {
                    Success = true,
                    Message = "Success",
        ...
        {
            return _context.dm_Xe!.Any(e => e.LOC_ID == LOC_ID && e.MA == MA);
        }

        private bool CarExistsID(string LOC_ID, string ID)
        {
            return _context.dm_Xe!.Any(e => e.LOC_ID == LOC_ID && e.ID == ID);
        }

        private bool CarExists(dm_Xe Car)
        {
            return _context.dm_Xe!.Any(e => e.LOC_ID == Car.LOC_ID && e.MA == Car.MA && e.ID != Car.ID);
        }
    }
}