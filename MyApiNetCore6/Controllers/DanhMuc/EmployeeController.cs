...
namespace MyApiNetCore6.Controllers
{
	...
	public class EmployeeController : ControllerBase
	{
		...
		public async Task<IActionResult> GetEmployee(string LOC_ID)
		{
			try
			{
				List<view_dm_NhanVien> lstValue = await (from e in _context.view_dm_NhanVien
		...
		public async Task<IActionResult> GetEmployee(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
		{
			try
			{
				...
				List<view_dm_NhanVien> lstValue = await (from e in _context.view_dm_NhanVien.Where((view_dm_NhanVien e) => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch)
														 orderby e.MA
														 select e).ToListAsync();
				return Ok(new ApiResponse
				{
		...
		public async Task<IActionResult> GetEmployee(string LOC_ID, string ID)
		{
			try
			{
				view_dm_NhanVien Employee = await _context.view_dm_NhanVien.FirstOrDefaultAsync((view_dm_NhanVien e) => e.LOC_ID == LOC_ID && e.ID == ID);
				if (Employee == null)
				{
					return Ok(new ApiResponse
					{
		...
		public async Task<IActionResult> PutEmployee(string LOC_ID, string MA, dm_NhanVien Employee)
		{
			try
			{
				...
				{
					view_dm_NhanVien check = await _context.view_dm_NhanVien.FirstOrDefaultAsync((view_dm_NhanVien e) => e.LOC_ID == Employee.LOC_ID && e.ID != Employee.ID && e.ID_TAIKHOAN == Employee.ID_TAIKHOAN);
					if (check != null)
					{
						return Ok(new ApiResponse
						{
				...
				view_dm_NhanVien OKEmployee = await _context.view_dm_NhanVien.FirstOrDefaultAsync((view_dm_NhanVien e) => e.LOC_ID == Employee.LOC_ID && e.ID == Employee.ID);
				return Ok(new ApiResponse
				{
					Success = true,
					Message = "Success",
		...
		public async Task<IActionResult> DeleteEmployee(string LOC_ID, string ID)
		{
			try
			{
				dm_NhanVien Employee = await _context.dm_NhanVien.FirstOrDefaultAsync((dm_NhanVien e) => e.LOC_ID == LOC_ID && e.ID == ID);
				if (Employee == null)
				{
					return Ok(new ApiResponse
					{
				...
				ApiResponse apiResponse = await ExecuteStoredProc.CheckDelete(Employee, Employee.ID, Employee.MA);
		...
		{
			return _context.dm_NhanVien.Any((dm_NhanVien e) => e.LOC_ID == LOC_ID && e.MA == MA);
		}

		private bool EmployeeExistsID(string LOC_ID, string ID)
		{
			return _context.dm_NhanVien.Any((dm_NhanVien e) => e.LOC_ID == LOC_ID && e.ID == ID);
		}

		private bool EmployeeExists(dm_NhanVien Employee)
		{
			return _context.dm_NhanVien.Any((dm_NhanVien e) => e.LOC_ID == Employee.LOC_ID && e.MA == Employee.MA && e.ID != Employee.ID);
		}
	}
}
