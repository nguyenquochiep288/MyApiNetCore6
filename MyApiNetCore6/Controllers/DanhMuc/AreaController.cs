...
namespace MyApiNetCore6.Controllers
{
	...
	public class AreaController : ControllerBase
	{
		...
		public async Task<IActionResult> GetArea(string LOC_ID)
		{
			try
			{
				List<dm_KhuVuc> lstValue = await (from e in _context.dm_KhuVuc
		...
		public async Task<IActionResult> GetArea(string LOC_ID, int Type, string KeyWhere = "", string ValuesSearch = "")
		{
			try
			{
				...
				List<dm_KhuVuc> lstValue = await (from e in _context.dm_KhuVuc.Where((dm_KhuVuc e) => e.LOC_ID == LOC_ID).Where(KeyWhere, ValuesSearch)
												  orderby e.MA
												  select e).ToListAsync();
				return Ok(new ApiResponse
				{
		...
		public async Task<IActionResult> GetArea(string LOC_ID, string ID)
		{
			try
			{
				dm_KhuVuc Area = await _context.dm_KhuVuc.FirstOrDefaultAsync((dm_KhuVuc e) => e.LOC_ID == LOC_ID && e.ID == ID);
				if (Area == null)
				{
					return Ok(new ApiResponse
					{
		...
		public async Task<IActionResult> PutArea(string LOC_ID, string MA, dm_KhuVuc Area)
		{
			try
			{
				...
				dm_KhuVuc OKArea = await _context.dm_KhuVuc.FirstOrDefaultAsync((dm_KhuVuc e) => e.LOC_ID == Area.LOC_ID && e.ID == Area.ID);
				return Ok(new ApiResponse
				{
					Success = true,
					Message = "Success",
		...
		{
			return _context.dm_KhuVuc.Any((dm_KhuVuc e) => e.LOC_ID == LOC_ID && e.MA == MA);
		}

		private bool AreaExistsID(string LOC_ID, string ID)
		{
			return _context.dm_KhuVuc.Any((dm_KhuVuc e) => e.LOC_ID == LOC_ID && e.ID == ID);
		}

		private bool AreaExists(dm_KhuVuc Area)
		{
			return _context.dm_KhuVuc.Any((dm_KhuVuc e) => e.LOC_ID == Area.LOC_ID && e.MA == Area.MA && e.ID != Area.ID);
		}
	}
}
